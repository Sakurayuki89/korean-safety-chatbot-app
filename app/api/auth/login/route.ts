...
import { getClientIP, createRateLimitResponse, RATE_LIMITS } from '@/lib/rate-limiter';
...
export async function POST(request: Request) {
  // 1. Rate limiting check - FIRST priority for security
  ...
  try {
    ...
    if (password === ADMIN_PASSWORD) {
      ...
      console.log(`[login] Successful login from IP: ${clientIP}`);
      return NextResponse.json({ success: true }, { status: 200 });
    } else {
      // Password is incorrect
      console.warn(`[login] Failed login attempt from IP: ${clientIP}`);
